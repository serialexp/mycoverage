...
function calculateSignature(lineData: LineData[] | undefined) {
  return lineData
    ?.map((i) => (i.type === "cond" && i.covered < i.total ? "P" : i.count > 0 ? "C" : "U"))
    .join()
}
...
const BranchFileDifferencePage: BlitzPage = () => {
    ...
    return (
      <pre
        ...
        dangerouslySetInnerHTML={{
          __html: str ? Prism.highlight(str, Prism.languages.typescript, "typescript") : "",
        }}
      />
    )
  }
  ...
  for (const line in baseCoverageData.coveragePerLine) {
    ...
    if (
      calculateSignature(baseCoverageData.coveragePerLine[line]?.coverageItems) !==
          ...
          <ReactDiffViewer
            ...
            renderGutter={(diffData) => {
              ...
              const baseSignature = calculateSignature(
                baseCoverageData.coveragePerLine[diffData.lineNumber]?.coverageItems
              )
              const signature = calculateSignature(
                coverageData.coveragePerLine[diffData.lineNumber]?.coverageItems
              )
              let changed = false
