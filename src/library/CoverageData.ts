...
export class CoverageData {
	...
	static fromInternalCoverage(coverage: InternalFileCoverage): CoverageData {
		...
		coverage.items.forEach((item) => {
			...
			Object.keys(item.hitsFromSource).forEach((index) => {
				...
				if (source) {
					hitsBySource[source] = [item.hitsFromSource[index]];
				}
			});
	...
	static fromCoberturaFile(
		...
		file.lines?.forEach((line) => {
			const value = branchLineIndex[line.number];
			if (!value) {
				return;
			}
			if (line.branch) {
				branchLineIndex[line.number] = value + 1;
				const hitsBySource = pullHitInfo(sources, "b", line.number, value);
				...
				statementLineIndex[line.number] = value + 1;
				const hitsBySource = pullHitInfo(sources, "s", line.number, value);
		...
		file.functions?.forEach((func) => {
			const value = functionLineIndex[func.number];
			if (!value) {
				return;
			}
			functionLineIndex[func.number] = value + 1;
			const hitsBySource = pullHitInfo(sources, "f", func.number, value);
				...
				newItems?.forEach((newItem, index) => {
					...
					if (existingItem) {
						...
						if (existingItem.type === "branch" && newItem.type === "branch") {
							const newConditions: Record<number, number> =
							...
							Object.keys(newItem.conditionals).forEach((conditional) => {
								newConditions[conditional] =
									...
									newItem.conditionals[conditional];
							});
