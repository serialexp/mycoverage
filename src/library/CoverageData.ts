...
export class CoverageData {
  ...
  public getLineSummary() {
    const summary: Record<number, "c" | "p" | "u"> = {}
        ...
        newItems?.forEach((newItem, index) => {
          ...
          if (existingItem) {
            ...
            if (existingItem.type === "branch" && newItem.type === "branch") {
              const newConditions: Record<number, number> = existingItem.conditionals
              Object.keys(newItem.conditionals).forEach((conditional) => {
                newConditions[conditional] =
                  (newConditions[conditional] ?? 0) + newItem.conditionals[conditional]
              })
