...
export async function getFileData(org: string, repo: string, ref: string, path: string) {
  const res = await (
    await getAppOctokit()
  ).repos.getContent({
    owner: org,
...
export async function areRefWorkflowsAllComplete(org: string, repo: string, ref: string) {
  ...
  const hasFailures = suites.data.check_runs.some(
    (suite) =>
      (suite.status !== "completed" || suite.conclusion === "failure") && suite.name !== "Coverage"
  )
