...
export class InternalCoverage {
  ...
  public locateDirectory(packageName: string) {
    ...
    let currentPackage: InternalDirectory | undefined = this.data.directories[0]?.children.find(
    ...
    while (currentPart && currentPackage && packageParts.length > 0) {
      ...
      const nextPackage: InternalDirectory | undefined = currentPackage.children.find(
        (dir) => dir.name === currentPart
      )
      if (!nextPackage) {
        return undefined
  ...
  private locateOrCreateDirectory(packageName: string): InternalDirectory {
    ...
    while (currentPart && currentPackage) {
      ...
      let nextPackage: InternalDirectory | undefined = currentPackage.children.find(
        (dir) => dir.name === currentPart
      )
      if (!nextPackage) {
