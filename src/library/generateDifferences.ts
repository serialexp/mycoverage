...
export async function generateDifferences(
  ...
  changedFileFileCoverageData.forEach((item) => {
    coverageData[item.id.toString("base64")] = CoverageData.fromProtobuf(item.coverageData)
  })

  changedFiles.forEach((item) => {
    if (item.next && !item.base) {
      item.nextFrom = coverageData[item.next.id.toString("base64")]?.getAllHitSources()
    }
    if (!item.next && item.base) {
      item.baseFrom = coverageData[item.base.id.toString("base64")]?.getAllHitSources()
    }
  })
