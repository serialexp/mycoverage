...
describe("InternalCoverage", () => {
  ...
  it("sorts packages after adding new intermediate ones", () => {
    ...
    expect(coberturaCoverage.data.directories[0]?.children[0]?.name).toEqual("src")
    expect(coberturaCoverage.data.directories[0]?.children[0]?.children[0]?.name).toEqual("super")

    coberturaCoverage.mergeCoverageString("action.mega", "action.ts", "stmt,2,3", "test2")
    coberturaCoverage.updateMetrics()

    expect(coberturaCoverage.data.directories[0]?.children[0]?.name).toEqual("action")
    expect(coberturaCoverage.data.directories[0]?.children[1]?.children[0]?.name).toEqual("super")
  })
  ...
  it("uses the basepath in combination with cwd to correct path locations in cobertura", async () => {
    ...
    for (const root of rootPermutations) {
      for (const source of sourcePermutations) {
        ...
        const packageNames = coberturaCoverage.flattenDirectories().map((p: any) => p.fileName)
...
end_of_record`,
        ...
        const packageNames = internalCoverage.flattenDirectories().map((p: any) => p.fileName)
...
end_of_record`,
    ...
    expect(internalCoverage.data.directories[0]?.files[0]?.name).toEqual("app.js")
    expect(internalCoverage.data.directories[0]?.children[0]?.files[0]?.name).toEqual("index.js")
  })
})
