  ...
  for (const lineText of lines) {
    ...
    switch (key) {
      ...
      case "FN": {
        ...
        if (!line || !name) throw new Error(`Missing line or function name on line ${i}`)
      ...
      case "FNDA": {
        ...
        if (!fn)
          throw new Error(`Could not find function "${functionName}" referenced on line ${i}`)
      ...
      case "DA": {
        ...
        if (!line || !hits) throw new Error(`Missing line or hit count on line ${i}`)
        record.lines.push({
          line: parseInt(line),
          hits: parseInt(hits),
        })
      ...
      case "BRDA": {
        ...
        if (!line || !block || !branch || !hits)
          throw new Error(`Missing line, block, branch or hit count on line ${i}`)
        ...
        lineData.branches[branch] =
          branchValue !== undefined ? branchValue + parseInt(hits) : parseInt(hits)
        break
      }
      case "LF":
