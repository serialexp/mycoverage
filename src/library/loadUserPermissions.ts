...
export const loadUserPermissions = async (userId: number, accessToken: string) => {
    ...
    .then(async (installations) => {
      ...
      const allInstallationRepositories = await Promise.all(
        installations.data.installations.map(async (installation) => {
          ...
          console.log(`Get page 1 of repositories for installation ${installation.id}`)
          ...
          while (page <= totalPages) {
            console.log(`Get page ${page} of repositories for installation ${installation.id}`)
            promises.push(
      ...
      for (const [owner, repositories] of repositoriesPerOwner) {
        ...
        const existingRepositoryNames = new Set(existingRepositories.map((r) => r.name))
        const newRepositories = repositories.filter((r) => !existingRepositoryNames.has(r.name))
