...
const combineCoverageForCommit = async (commit: Commit) => {
  ...
  for (const test of Object.values(lastOfEach)) {
    const files = test.PackageCoverage.map((pkg) => pkg.FileCoverage?.length ?? 0).reduce(
      (a, b) => a + b,
      0
    )
    log(
    ...
    if (satisfied.isOk && allFinished) {
      ...
      if (prWithLatestCommit) {
        ...
        if (allComplete) {
          ...
          log(`commit: Github tasks not yet finished ${commit.id}, skipping PR update`)
        }
      } else {
        log(`commit: No PR found for commit id ${commit.id}, skipping PR update`)
      }
    } else {
