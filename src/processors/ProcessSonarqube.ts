...
export const sonarqubeWorker = new Worker<{
  ...
  async (job) => {
    ...
    try {
      ...
      log(`Posted ${issues.length} issues, ${hashes.length} of which have a hash`)
      ...
      issues.forEach((issue) => {
        ...
        if (!hashDeduplicated[issue.hash]) {
          ...
          log(`Duplicate key for ${issue.hash}`, issue, hashDeduplicated[issue.hash])
          throw new Error("Cannot handle file with duplicates")
        }
      })
      log(`${Object.keys(hashDeduplicated).length} unique hashes in submitted issues`)
      ...
      log(`Found ${existingIssues.length} existing issues in DB with similar hashes`)
      ...
      log(`After creation of new issues, found ${refreshedIssues.length} issue ids based on hashes`)
      ...
      log(`${existingLinks.length} existing issues already linked to this commit`)
        ...
        if (fileId && fileIdBuffer) {
          if (
            !existingFileIssues.find(
              (existing) =>
                existing.fileCoverageId === fileIdBuffer && existing.codeIssueId === issue.id
            )
          ) {
      ...
      log(`Found ${newFileCoverages.length} new code issues to attach to file coverage`)
