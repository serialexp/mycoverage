		...
		try {
			...
			if (group && project) {
				...
				if (satisfied.isOk && allFinished) {
					...
					if (!commit)
						throw new Error("Cannot combine coverage without a commit");
					...
					log(
						`commit: Found ${Object.keys(lastOfEach).length} tests to combine.`,
					);
					...
					await mydb.commit.update({
						...
						data: {
							...
							coveredStatements:
								coverage.data.coverage.metrics?.coveredstatements ?? 0,
							coveredConditionals:
								coverage.data.coverage.metrics?.coveredconditionals ?? 0,
							coveredMethods:
								coverage.data.coverage.metrics?.coveredmethods ?? 0,
							coveredElements:
								coverage.data.coverage.metrics?.coveredelements ?? 0,
							coveredPercentage: coveredPercentage(
								coverage.data.coverage.metrics,
							),
						},
					});
