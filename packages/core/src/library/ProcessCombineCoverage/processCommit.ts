import type { PrismaClient, Commit } from "@mycoverage/db"
...
import { insertCoverageData } from "@mycoverage/core/library/insertCoverageData"
...
export async function processCommit(args: {
  ...
  if (group && project) {
    ...
    if (prWithLatestCommit) {
      log(
        `Commit for PR ${prWithLatestCommit.id} (Github ${prWithLatestCommit.sourceIdentifier}), ref ${commitToUse?.ref}`,
      )
    } else {
    ...
    if (satisfied.isOk && allFinished) {
      ...
      if (prWithLatestCommit && commitToUse) {
