...
export const processAllInstances = async (job: Job<ProcessCombineCoveragePayload, any, string>) => {
  ...
  for (let i = 0; i < all.length; i++) {
    ...
    for (let j = 0; j < test.TestInstance.length; j++) {
      ...
      console.log(
        `test: Merged ${packages} packages and ${files} files for instance index ${test.testName}:${
          testInstance.index
    ...
    console.log(
      "test: Test instance combination with test instances result: " +
